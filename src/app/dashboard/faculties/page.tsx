...
export default function FacultiesPage() {
  ...
  const [userForm, setUserForm] = useState({ name: "", login: "", password: "", role: "TEACHER", facultyId: "", departmentId: "" });
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setUserForm({ name: "", login: "", password: "", role: "TEACHER", facultyId: "", departmentId: "" });
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                       ...
                       <button 
                         onClick={(e) => {
                            e.stopPropagation();
                            setUserForm({ name: "", login: "", password: "", role: "DEAN", facultyId: faculty.id, departmentId: "" });
                      ...
                      <li key={dep.id} className="group/item flex items-center justify-between gap-3 text-sm bg-white px-4 py-3.5 rounded-2xl border border-slate-100 shadow-sm hover:border-indigo-200 hover:shadow-md transition-all cursor-default relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-1 relative z-10 opacity-60 group-hover/item:opacity-100 transition-opacity">
                          <button 
                            onClick={(e) => { 
                               e.stopPropagation(); 
                               setUserForm({ name: "", login: "", password: "", role: "HOD", facultyId: faculty.id, departmentId: dep.id });
                               setIsUserModalOpen(true);
                            }}
             ...
             <form onSubmit={handleAddUser} className="p-8 space-y-4">
               ...
               <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                   ...
                   <input type="text" required value={userForm.login} onChange={e => setUserForm({...userForm, login: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="johndoe12" />
                 </div>
               </div>
               <div>
