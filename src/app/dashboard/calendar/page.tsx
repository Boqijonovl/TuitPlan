...
export default function CalendarPage() {
  ...
  const CustomToolbar = (toolbar: any) => {
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between p-4 mb-4 bg-slate-50 rounded-xl border border-slate-100">
        <div className="flex items-center gap-2 mb-4 sm:mb-0">
          <button onClick={goToBack} className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 hover:text-blue-600 transition-colors shadow-sm"><ChevronLeft className="w-5 h-5"/></button>
          ...
          <button onClick={goToNext} className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 hover:text-blue-600 transition-colors shadow-sm"><ChevronRight className="w-5 h-5"/></button>
        </div>
        <h2 className="text-xl font-bold text-slate-900 capitalize">{headerText}</h2>
        <div className="flex items-center gap-2 mt-4 sm:mt-0 bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
          <button onClick={() => toolbar.onView('month')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${toolbar.view === 'month' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>Oy</button>
          <button onClick={() => toolbar.onView('week')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${toolbar.view === 'week' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>Hafta</button>
          <button onClick={() => toolbar.onView('day')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${toolbar.view === 'day' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>Kun</button>
          <button onClick={() => toolbar.onView('agenda')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${toolbar.view === 'agenda' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>Ro'yxat</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-4 h-[calc(100vh-7rem)] flex flex-col">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex-1 p-4 sm:p-6 relative">
        {loading && (
           ...
           <div className="bg-white rounded-xl shadow-sm w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
             ...
             <div className="p-5 space-y-4">
                <div>
                  ...
                  {selectedEvent.plan?.title && (
                    <p className="text-sm text-blue-600 font-medium mt-1 inline-flex items-center gap-1.5 bg-blue-50 px-2.5 py-1 rounded-md">
             ...
             <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
               ...
               <button 
                 ...
                 className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold shadow-sm transition-colors"
               >
                 Vazifaga o'tish
               </button>
             </div>
